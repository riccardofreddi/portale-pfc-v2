...
interface Notifica { id: string; type: string; text: string; detail: string; ts: string; read: boolean; year?: string | null; folder?: string | null }

const NOTIF_ICONS: Record<string, string> = { documento_nuovo: '📄', messaggio: '💬', avviso: '📢', richiesta_upload: '📤', upload_confermato: '✅' }
...
export function ClienteArea() {
  ...
  // Loading mentre verifica manutenzione
  if (maintenance === null) {
    return (
        ...
        <main className="flex-1 flex items-center justify-center p-4">
          <div className="text-center bg-gradient-to-br from-amber-50 to-amber-100 border-2 border-amber-400 rounded-3xl p-8 sm:p-12 max-w-2xl shadow-xl">
            ...
            <div className="border-t border-amber-300 pt-6 mt-6">
              <p className="text-amber-700 text-sm">Per urgenze, contatta lo studio.<br/>Grazie per la pazienza. 🙏</p>
            </div>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {nNotifiche > 0 && (
            <div className="mt-3 flex items-center gap-3 flex-wrap">
              <button onClick={() => setShowNotifPanel(!showNotifPanel)} className="bg-white text-red-600 text-sm font-bold px-3 py-1.5 rounded-full shadow flex items-center gap-1.5 hover:bg-red-50 transition-colors">
                <Bell className="h-3.5 w-3.5" />{nNotifiche} notifich{nNotifiche > 1 ? 'e' : 'a'} non lett{nNotifiche > 1 ? 'e' : 'a'}
              </button>
        ...
        {avvisi.length > 0 && (<div className="space-y-2 mb-6">{avvisi.map((a) => (<div key={a.id} className="bg-amber-50 border border-amber-300 border-l-4 border-l-amber-500 rounded-lg p-3"><p className="text-xs text-amber-700 font-semibold mb-1">📢 Comunicazione dello Studio · {new Date(a.timestamp).toLocaleDateString('it-IT', { day: 'numeric', month: 'short', year: 'numeric' })}</p><p className="text-sm text-amber-900 whitespace-pre-wrap">{a.text}</p></div>))}</div>)}

        <div className="border-b border-slate-200 mb-6">
          <nav className="-mb-px flex gap-1 overflow-x-auto" aria-label="Tabs">
            {TABS.map((t) => { const Icon = t.icon; const active = clienteTab === t.id; const badge = t.id === 'messaggi' && nMsgNonLetti > 0 ? nMsgNonLetti : 0; return (
              <button key={t.id} onClick={() => setClienteTab(t.id)} className={cn('flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap relative', active ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300')}>
                <Icon className="h-4 w-4" />{t.label}{badge > 0 && <span className="bg-red-500 text-white text-xs font-bold rounded-full px-2 py-0.5 min-w-[20px] text-center">{badge}</span>}
              </button>) })}
          </nav>
        </div>
