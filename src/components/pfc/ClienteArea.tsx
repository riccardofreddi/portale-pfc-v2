...
export function ClienteArea() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setNNotifiche((n.notifiche as Array<{ read: boolean }>).filter((x) => !x.read).length)
      setNMsgNonLetti((m.messaggi as Array<{ read: boolean }>).filter((x) => !x.read).length)
      setAvvisi(a.avvisi)
    })
  }, [user, clienteTab])

  async function handleSegnaLette() {
    try { await api.notifiche.segnaLette(); setNNotifiche(0); toast.success('Notifiche segnate come lette') }
        ...
        <div className="bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl p-6 mb-6 text-white shadow-lg">
          ...
          {nNotifiche > 0 && (
            <div className="mt-3 flex items-center gap-3">
              ...
              <Button variant="secondary" size="sm" onClick={handleSegnaLette}>Segna come lette</Button>
            </div>
          )}
        </div>
        {avvisi.length > 0 && (
