...
import { LogOut, Wrench, Shield, User as UserIcon, Bell, BellRing, ChevronDown } from 'lucide-react'
...
export function TopBar() {
  const { user, setUser, nNotifiche, setShowNotifPanel } = usePfcStore()
  const [maintenance, setMaintenance] = useState(false)
  // Notifiche browser lato STUDIO (admin): finora esisteva solo per i clienti
  // (ClienteArea). Il primo click chiede il permesso al browser, una volta sola;
  // quando le notifiche sono attive, lo stesso pulsante diventa "Prova notifica".
  const push = usePushNotifications(user?.role === 'admin')
  ...
  // Prima attivazione: subscribe + permesso browser. Se gia' attive: invia una
  // notifica di prova cosi' lo studio vede subito il fumetto in basso a destra.
  async function handlePush() {
    if (push.subscribed) {
      try {
        const r = await push.test()
        if (r.ok) toast.success('Notifica di prova inviata: guarda in basso a destra dello schermo')
        else toast.error(`Test non riuscito: nessuna notifica inviata${r.msg ? ` (${r.msg})` : ''}`)
      } catch {
        toast.error('Test non riuscito')
      }
      return
    }
    try {
      await push.subscribe()
      toast.success('Notifiche attive su questo browser')
    } catch {
      toast.error("Permesso non concesso: clicca sull'icona accanto all'indirizzo e imposta Notifiche su Consenti")
    }
  }
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 h-[60px] flex items-center justify-between px-3 sm:px-6 lg:px-8 shadow-sm">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        {/* Notifiche browser per lo studio: prima "Attiva notifiche", poi "Prova notifica" */}
        {user.role === 'admin' && push.supported && (
          <button
            onClick={handlePush}
            disabled={push.loading}
            className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-[10px] text-xs font-semibold transition-all flex-shrink-0 disabled:opacity-50 ${
              push.subscribed
                ? 'bg-emerald-50 text-emerald-700 border border-emerald-300 hover:bg-emerald-100'
                : 'bg-emerald-500 text-white border border-emerald-500 shadow-sm hover:bg-emerald-600'
            }`}
          >
            {push.subscribed ? <BellRing className="h-3.5 w-3.5 flex-shrink-0" /> : <Bell className="h-3.5 w-3.5 flex-shrink-0" />}
            <span className="hidden sm:inline">{push.subscribed ? 'Prova notifica' : 'Attiva notifiche'}</span>
          </button>
        )}

        {/* Notifications bell - badge numerico per i clienti */}
        {user.role === 'client' && (
          <button
            onClick={() => setShowNotifPanel(true)}
            className="relative flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-xl hover:bg-slate-100 transition-all border-1.5 border-transparent hover:border-slate-200 flex-shrink-0"
            aria-label="Notifiche"
          >
            <Bell className="h-5 w-5 text-slate-600" />
            {nNotifiche > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] sm:text-[11px] font-bold rounded-full min-w-[18px] sm:min-w-[20px] h-[18px] sm:h-[20px] px-1 flex items-center justify-center shadow-md border-2 border-white">
