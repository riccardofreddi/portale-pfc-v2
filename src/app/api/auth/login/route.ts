...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!v.ok) {
      return NextResponse.json({ ok: false, error: v.msg }, { status: 400 })
    }
    ...
    if (waitSec > 0) {
      return NextResponse.json(
        { ok: false, error: `Troppi tentativi falliti. Riprova tra ${waitSec} secondi.` },
        { status: 429 }
      )
    }
    ...
    if (!user || !verifyPassword(password, user.passwordHash)) {
      ...
      if (username !== DEFAULT_ADMIN_USER) {
        await logAudit(username, 'LOGIN_FAILED', 'Password sbagliata')
      }
      const msg =
        waitAfter > 0
          ? `Troppi tentativi falliti. Riprova tra ${waitAfter} secondi.`
    ...
    if (username !== DEFAULT_ADMIN_USER) {
      await logAudit(username, 'LOGIN_SUCCESS', 'Accesso eseguito')
    }
